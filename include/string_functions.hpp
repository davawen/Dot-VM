...
/// Iterates over a string and calls a function for every characters in it, but ignores anything wrapped in double
/// quotes
/// @param func function used with signature `int(std::string &, size_t &)`. Non zero return value indicates to break
/// out of loop
template <typename F>
void iterate_ignore_quotes(std::string &str, F func, size_t pos = 0) {
	static_assert(
		std::is_invocable<F, std::string &, size_t &>::value,
		"Invalid function signature passed to iterate_ignore_quotes, must respect int(std::string &, size_t &)");
