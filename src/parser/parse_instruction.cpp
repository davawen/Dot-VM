	...
	for (; str[index] != '\0'; index++) {
		...
		if (chr == '\\') {
			...
			if (chr == '\0')
				compile_error(
					0,
					"Unterminated escape sequence");  // TODO: Figure out how to pass line / char number appropriately
	...
	while (idx < tokens.size()) {
		...
		for (int i = 0; i < numArgs; i++) {
			...
			case Token::OPERATOR:
				...
				break;
			case Token::LITERAL:
				if (instructionType == Instruction::Type::LABEL || instructionType == Instruction::Type::JUMP ||
				    instructionType == Instruction::Type::CALL ||
				    (instructionType == Instruction::Type::IFEQ && i == 0)) {
				...
				break;
			default:
				compile_error(*curToken.line,
				              fmt::format("Unexpected token encountered: {}, {}", curToken.type, curToken.value));
				break;
			}
		}
