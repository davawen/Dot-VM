...
void search_macro(std::vector<Line> &output, size_t idx, const MacroMap &macros) {
	...
	while ((macro_start = find_ignore_quotes(str, "#(", macro_start)) != std::string::npos) {
		...
		// Could have checked for invalid name earlier, but it would have probably triggered in the case of an
		// unfinished macro, so it's better to see after it if(macros.find(name) == macros.end()) compile_error(line,
		// fmt::format("Uknown macro in expansion: {}", name));
